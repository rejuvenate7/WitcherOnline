...
function MP_SU_removeCustomPinByIndex(index: int) {
  var manager: MP_SUMP_Manager;

  manager = MP_SUMP_getManager();
  manager.mappins.Erase(index);

  MP_SU_removeMinimapPin(index);
}

function MP_SU_removeMinimapPin(old_index: int) {
...
function MP_SU_removeCustomPinByPredicate(predicate_runner: MP_SU_PredicateInterfaceRemovePin) {
  ...
  while (i >= 0) {
    ...
    if (predicate_runner.predicate(current_pin)) {
      MP_SU_removeCustomPinByIndex(i);
    }
 ...
 * needs.
 */
abstract class MP_SU_PredicateInterfaceRemovePin {
  /**
